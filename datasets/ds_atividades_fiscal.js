function createDataset(fields, constraints, sortFields) {
	...
	for (var i = 0; i < constraints.length; i++) {
		...
		if (constraints[i].fieldName == "CD_MATRICULA") {
			descricao = constraints[i].initialValue;
		}
	}

	var campos = new Array("QUANTIDADE", "FULL_NAME", "LOGIN", "CD_MATRICULA");
	...
	var arrayConstraints = new Array(c1, c2, c3);
	...
	if (datasetRM == null || datasetRM == undefined) {
		...
		for (var i = 0; i < datasetRM.rowsCount; i++) {
			...
			dataset.addRow(new Array(QUANTIDADE, FULLNAME, LOGIN, CDMATRICULA));
		}
